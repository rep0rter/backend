...
function getDateString(): string {
  ...
  return `${chalk.dim(`${month}/${date}`)} ${chalk.cyan(
    `${hours}:${minutes}:${seconds}`
  )}`;
}

function formatString(input: string) {
  return input + " ".repeat(Math.max(0, 20 - input.length));
}

export function info(message: string, sender: string): void {
  console.log(
    `${getDateString()} ${formatString(chalk.green(sender))}${chalk.blue(
      "info"
    )} ${message}`
  );
}
...
export function error(message: string, sender: string, error?: unknown): void {
  console.error(
    `${getDateString()} ${formatString(chalk.green(sender))}${chalk.red(
      "error"
    )} ${message}`
  );
